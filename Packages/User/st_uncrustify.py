...
class SublimeUncrustifyOnSave(sublime_plugin.EventListener):
  def on_pre_save(self, view):
    # self.settings = sublime.load_settings('SublimeUncrustify.sublime-settings')
    # if self.settings.get('run_on_save'):
      view.run_command("sublime_uncrustify", {"save": False, "error": False})

class SublimeUncrustifyCommand(sublime_plugin.TextCommand):
  ...
  def beautify_buffer(self, edit):
    ...
    if buffer_text != "":
      ...
      beautified_buffer = self.pipe("/usr/local/bin/uncrustify -c /Developer/adec_coding_std.cfg -l CPP", buffer_text)
      # fix_lines = beautified_buffer.replace(os.linesep,'\n')
      fix_lines = beautified_buffer
      # self.check_valid_output(fix_lines)
      if fix_lines != "":
        self.view.replace(edit, buffer_region, fix_lines)
      else:
        sublime.error_message("Error: uncrustify returned a ZERO LENGTH result!!!")
  ...
  #   ruby_script = os.path.join(sublime.packages_path(), 'SublimeUncrustify', 'lib', script_name)
  ...
  def is_c_file(self):
    # file_patterns = self.settings.get('file_patterns') or ['\.c', '\.cpp', '\.h', '\.hpp', '\.C', '\.CPP', '\.H', '\.HPP']
    file_patterns = ['\.c', '\.cpp', '\.h', '\.hpp', '\.C', '\.CPP', '\.H', '\.HPP']
